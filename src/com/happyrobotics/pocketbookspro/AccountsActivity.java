...
public class AccountsActivity extends SherlockActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		final Intent transactionIntent = new Intent(this,
		...
		AccountAdapter adapter = new AccountAdapter(this,
				R.layout.accounts_activity_listview_row, cursor, from, to);
		...
		list.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> a, View v, int position,
				...
				// Log.d(TAG, "" +
				// cursor.getString(cursor.getColumnIndex(AccountData.ACCOUNT_NAME)
				// - 1));

				startActivity(transactionIntent.putExtra(
	...
	public void onResume() {
		...
		if (hasAccounts) {
			...
			sum = new BigDecimal(accountsSum.getString(accountsSum
					.getColumnIndex(AccountData.ACCOUNT_BALANCE)));
		}
	...
	//
	// sum = new
	// BigDecimal(accountsSum.getString(accountsSum.getColumnIndex(AccountData.ACCOUNT_BALANCE)));
	//
	// } else {
	// hasAccounts = false;
	// sum = BigDecimal.ZERO;
